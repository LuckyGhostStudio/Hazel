...
namespace Hazel
{
	...
	namespace Utils
	{
		...
		static GLenum HazelTextureFormatToGL(FramebufferTextureFormat format)
		{
			...
			HZ_CORE_ASSERT(false, "No format");
			return 0;
		}
	}
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		if (m_ColorAttachments.size() > 1) {
			HZ_CORE_ASSERT(m_ColorAttachments.size() <= 4, "m_ColorAttachments.size() > 4");	//颜色缓冲区不超过4
	...
	{
		HZ_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(), "index越界！");
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0, 
			Utils::HazelTextureFormatToGL(spec.TextureFormat), GL_INT, &(value));	//清除attachmentIndex号颜色缓冲区的值为value
	}
}